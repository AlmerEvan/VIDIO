...
<?php 
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM ibmenu WHERE idmenu = $id";
        $item = $db->getITEM($sql);
        $idkategori = $item['idkategori'];

    }

    $row = $db->getALL("SELECT * FROM ibkategori ORDER BY kategori  ASC");
...
<?php 
    ...
    if (isset($_POST['simpan'])) {
        ...
        $sql = "UPDATE ibmenu SET idkategori = $idkategori, menu = '$menu', gambar = '$gambar', harga = '$harga'  WHERE idmenu = $id";
